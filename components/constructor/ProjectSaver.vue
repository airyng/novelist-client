...
export default {
    ...
    methods: {
        ...
        async save(){
            ...
            var response = await axios.post('/api/game/save', data)
        ...
        checkExitExistence () { // проверяем, если ли в новелле хотя бы один выход
            ...
            var gameHasExit = false

            for (let index = 0; index < this.scenes.length; index++) {
                const scene = this.scenes[index]
                
                if(this.isSceneHasExit( scene ))
                    gameHasExit = true
            }

            if(!gameHasExit)
                this.addError("В новелле нет ни одной кнопки выхода")
        },
        checkMainSettings(){
        ...
        checkSeparatedScenes(){
            var scenesIDsWithTransitions = this.getScenesIDsWithTransitions()
        ...
        getScenesIDsWithTransitions(){ // получаем id сцен, на которые ведут все имеющиеся экшены переходов
            ...
            for (var index = 0; index < this.scenes.length; index++) {
                ...
                for (let actionIndex = 0; actionIndex < scene.actions.length; actionIndex++) {
                    ...
                    if(action.to && typeof action.to == "number")
                        sceneIDs.push( action.to )
                }
            }
            return sceneIDs
        },
