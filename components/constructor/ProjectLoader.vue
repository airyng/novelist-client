...
export default {
    ...
    methods: {
        ...
        validateGameState ( gameData ) {
            ...
            if(gameData.author != this.user.id && this.$route.name == 'edit-game')
                this.$router.push({name: '423'})
            // Если пытаются поиграть в черновик
            if(gameData.status == 'draft' && this.$route.name == 'play-game')
                this.$router.push({name: '404'})
            // Если в тестдрайв пытается зайти не автор
            if(gameData.status == 'test_drive' && gameData.author != this.user.id)
                this.$router.push({name: '404'})
        },
